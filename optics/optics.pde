...
void draw() {
  ...
  for(int i=0; i<totalRays; i++){
    ...
    for(int s=0; s < 6/*sides.length*/; s++){
  ...
  if(frameCount < 60*30){
    println(frameCount+" ");
    saveFrame("/Users/artem/work/creative-code/prizm/prism__####.png");
  }
}
...
//=======================================
PVector[] renderRay(boolean fromOutside, int i, float waveLength, PVector[] ray, PVector[] wall){
  strokeWeight(1.5);
  int [] clr = waveLengthToRGB(waveLength * 1e3);
  PVector r0 = ray[0];
  PVector r1 = ray[1]; //<>//
  
  stroke(clr[0],clr[1],clr[2], strokeAlpha);
  //line(r0.x, r0.y, r1.x, r1.y);
  PVector xx = intersection( wall[0], wall[1], r0, r1);
  if(xx!=null){
    fill(255, 0, 0, 80);
  ...
  PVector r_base = PVector.sub(r1, r0);
  PVector p_base = PVector.sub(wall[0], wall[1]);
  float a = -(PI/2 - PVector.angleBetween(r_base, p_base));
  ...
  double t  =  asin(( 1.0 / ri_by_wl(waveLength, (float)ri)) * sin(a));
  
  
  PVector refracted = new PVector(width, 0);
  refracted.rotate((float)t);
  refracted = PVector.add(refracted, r1);
  
  
  stroke(clr[0],clr[1],clr[2], strokeAlpha);
  line(r1.x, r1.y, refracted.x, refracted.y);
...
float ri_by_wl(float  waveLength, float refractive_index){
  ...
  //println(waveLength2+" " +n);
  return (float)n;//refractive_index + c /(waveLength * waveLength);
